...
Operations::MenuResult Operations::menu_window(QString title, QStringList parameters_labels, QStringList &parameters, QStringList output_labels, QStringList &output, QString help, bool accept_blank_parameters)
{
	...
	std::vector<QLabel*> parameters_label(parameters_labels.size());
	std::vector<QLineEdit*> parameters_lineedit(parameters_labels.size());
	...
	std::vector<QLabel *> output_label(parameters_labels.size());
	std::vector<QLineEdit *> output_lineedit(parameters_labels.size());
	...
	if(output_labels.size()>0)
	{
		...
		for (int i = 0; i < output_labels.size(); ++i)
		{
			...
			output_label.push_back(new QLabel(output_box));
			output_label[i]->setText(output_labels.at(i));
			output_layout->addWidget(output_label[i]);
			output_label[i]->show();
			
			output_lineedit.push_back(new QLineEdit(output_box));
